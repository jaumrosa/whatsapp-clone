...
export class Message extends Model {
        ...
        switch(this.type){
            ...
            case 'audio':
                ...
                audioEl.ontimeupdate = e => {
                    ...
                    if (audioEl.paused) {
                        audioDuration.innerHTML = Format.toTime(this.duration * 1000); // ← total quando pausado
                        btnPlay.show();
                        btnPause.hide();
                    } else {
                        audioDuration.innerHTML = Format.toTime(audioEl.currentTime * 1000); // ← atual quando tocando
                        btnPlay.hide();
                        btnPause.show();
                    }
                }
    ...
    static sendAudio(chatId, from, file, metadata, photo){
        return Message.send(chatId, from, 'audio', '').then(msgRef => {
            Message.upload(file,from).then(snapshot => {
                getDownloadURL(snapshot).then(downloadFile => {
                    setDoc(msgRef, {
        ...
        Message.send(chatId, from, 'document', '').then(msgRef => {
            ...
            getDownloadURL(snapshot).then(downloadFile => {  
                if(filePreview) {
                    ...
                    getDownloadURL(snapshot2).then(downloadPreview => {
                        setDoc(msgRef, {
                            content: downloadFile,    
    ...
    return new Promise ((s , f) => {
        addDoc(Message.getRef(chatId), {    
            ...
            return updateDoc(result, { status: 'sent' }).then(() => s(result))
            }).catch(f)
        });
    }
